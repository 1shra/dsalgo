...
bool ATS(Node *root, Node *boot)
{
    ...
    {
        Node *c1 = root->vec[i];
        Node *c2 = root->vec[root->vec.size() - 1 - i];
        if (ATS(c1, c2) == false)
        {
            return false;
        }
...
int main()
{
    ...
    int brr[] = {1, 2, 5, -1, -1, 6, 7, -1, 8, 70, -1, 80, -1, -1 - 1, 110, 120, -1, 90, -1, -1, -1};
    int n = sizeof(arr) / sizeof(arr[0]);
    int m = sizeof(brr) / sizeof(brr[0]);
    Node *root = buildTree(arr, n);
    Node *boot = buildTree(brr, m);
    traversalWithLevel(boot);
    cout<<endl;
    traversalWithLevel(root);
    cout << "Node To root Path" << endl;
    cout << ATS(root, boot) << endl;
}